...
We are now ready for the power calculations. The needed function is \texttt{SsPowerGivenJK} (for ``sample size for given number of readers J and cases K''):
...
{\centering \includegraphics{19-froc-sample-size_files/figure-latex/unnamed-chunk-18-1} 
...
The fitting model is defined by \texttt{muNH} = 3.3121491, \texttt{lambdaNH} = 1.714368 and \texttt{nuNH} = 0.7036564 and \texttt{lesDistr\$Freq} = 0.69, 0.2, 0.11. The effect size scale factor is \texttt{scaleFactor} = 2.169337. These are identical to the Part I values.
...
The following example is for ROC effect size = 0.035 (line 1), 5 readers and 100 cases (line 4) in the \textbf{pivotal study}. The function \texttt{SsPowerGivenJK} returns the power for specified effect size, numbers of readers J, cases K and the just computed variance components (supplied as a \texttt{list} variable, lines 12 - 18).
...
The needed function \texttt{SsSampleSizeKGivenJ} (for number of cases K for desired power for given number of readers J) is shown below. If \texttt{dataset} is set to \texttt{NULL} then the effect size must be specified.
...
In the following code the function \texttt{SsSampleSizeKGivenJ} returns the number of cases needed for 80 percent power for the wAFROC effect size = 0.076 and wAFROC variance components as just computed above.
