...
\item
  The \texttt{x\$lesions\$perCase} member is a vector containing the number of lesions in each diseased case, i.e., 2, 1, 3, 2, 1 in the current example.
\item
...
Focus on the 115 diseased cases: the numbers of lesions in individual cases is contained in \texttt{x\$lesions\$perCase}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{LD }\OtherTok{\textless{}{-}} \FunctionTok{UtilLesDistr}\NormalTok{(x)}
\NormalTok{LD}
...
\item
  The \texttt{UtilLesDistr()} function returns an array with two columns and number of rows equal to the number of \emph{non-zero} values of lesions per case.
\item
  The first column (\texttt{lesID}) contains the number of non-zero values of lesions per case, 14 in the current example.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{sum}\NormalTok{(}\FunctionTok{UtilLesDistr}\NormalTok{(x)}\SpecialCharTok{$}\NormalTok{Freq)}
...
\section{Definition of the lesion weights}\label{quick-start-froc-data-lesion-weights}}

\begin{itemize}
\tightlist
\item
  This is returned by \texttt{UtilLesWghtsDS()} and \texttt{UtilLesWghtsLD()}.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lesWghtDistr }\OtherTok{\textless{}{-}} \FunctionTok{UtilLesWghtsDS}\NormalTok{(x, }\AttributeTok{relWeights =} \DecValTok{0}\NormalTok{)}
...
Given the variance components the sample size functions implemented in \texttt{RJafroc} (function names beginning with \texttt{Ss}) allow one to estimate the number of readers and cases necessary to detect (i.e., reject the null hypothesis) the modality AUC difference with probability no smaller than the Type II error rate \(\beta\), typically chosen to be 20 percent - corresponding to at least 80 percent statistical power - while controlling the Type I error rate to no greater than \(\alpha\), typically chosen to be 5 percent.
...
\item
  If the data uses continuous ratings one bins it as follows using \texttt{rocDatasetB\ \textless{}-\ DfBinDataset(rocDataset,\ opChType\ =\ "ROC")}. The default number of bins should be used. Unlike binning using arbitrarily set thresholds the thresholds found by \texttt{DfBinDataset()} are unique as they maximize ROC-AUC.
\item
  If there are more than two treatments in the pilot dataset, \texttt{dataset04}, one extracts those treatments that represent ``almost'' null hypothesis data (in the sense of similar ROC-AUCs). For example \texttt{datasetNH\ \textless{}-\ DfExtractDataset(dataset,\ trts\ =\ c(1,2))} extracts treatments 1 and 2. More than two treatments can be used if they have similar AUCs as the median finding procedure described below benefits from more data. However, the final sample size predictions are restricted to two treatments in the pivotal study.
\item
  The reason for using RSM parameter values only for the first two treatments is that these were found \citep{zanca2009evaluation} to be almost equivalent (more precisely, the NH could not be rejected for the first two treatments) so it makes sense to regard them as ``almost'' NH treatments.
\item
  If the original data is FROC, convert it to ROC using \texttt{rocDatasetNH\ \textless{}-\ DfFroc2Roc(datasetNH)}: this is because \textbf{the RSM only fits binned ROC data}.
\item
  For each treatment and reader the ROC data is fitted by \texttt{FitRsmRoc()}, see example below, yielding estimates of the RSM \emph{physical} parameters \(\mu, \lambda, \nu\) (not the \emph{intrinsic} values \(\mu, \lambda', \nu'\)). See Chapter TBA for the distinction between physical and intrinsic parameters.
\item
  The following code block defines the pilot FROC data \texttt{frocDataNH} and \texttt{rocDataNH}. The latter is the highest-rating ROC dataset inferred from the FROC dataset using \texttt{DfFroc2Roc()}.
\end{itemize}
...
\CommentTok{\# Following line is unnecessary as the data is already binned}
...
The next code block determines \texttt{lesDistr}, the lesion distribution dataframe, see Section TBA, as the RSM fitting algorithm needs to know how lesion-rich the dataset is: the predicted ROC-AUC depends on the lesion-richness of the dataset.
...
Note that \texttt{lesDistr} is determined from the FROC NH dataset as lesion distribution information is lost upon conversion to an ROC dataset.

For this dataset \texttt{Lmax} is 3, and fraction 0.69 of diseased cases have one lesion, fraction 0.2 of diseased cases have two lesions and fraction 0.11 of diseased cases have three lesions. The \texttt{PlotRsmOperatingCharacteristics} function used below sets \texttt{relWeights\ =\ 0} ensuring equally weighted lesions.

The next code block determines the number of treatments and readers (\texttt{I} and \texttt{J}) from the dimensions of the \texttt{frocDataNH\$ratings\$NL} array (lines 1 and 2). It creates (line 3) an array \texttt{RsmParmsNH} to hold the RSM fitted NH parameters. For each treatment and reader it applies the fitting algorithm \texttt{FitRsmRoc()} (lines 4 - 11). The first three returned values are \texttt{mu}, \texttt{lambda} and \texttt{nu}, corresponding to the physical RSM parameters \({\mu}\), \({\lambda}\) and \({\nu}\).

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{I }\OtherTok{\textless{}{-}} \FunctionTok{dim}\NormalTok{(frocDataNH}\SpecialCharTok{$}\NormalTok{ratings}\SpecialCharTok{$}\NormalTok{NL)[}\DecValTok{1}\NormalTok{]}
\NormalTok{J }\OtherTok{\textless{}{-}} \FunctionTok{dim}\NormalTok{(frocDataNH}\SpecialCharTok{$}\NormalTok{ratings}\SpecialCharTok{$}\NormalTok{NL)[}\DecValTok{2}\NormalTok{]}
...
\NormalTok{    x1 }\OtherTok{\textless{}{-}} \FunctionTok{FitRsmRoc}\NormalTok{(rocDataBinNH, }\AttributeTok{trt =}\NormalTok{ i, }\AttributeTok{rdr =}\NormalTok{ j, lesDistr}\SpecialCharTok{$}\NormalTok{Freq)}
\NormalTok{    RsmParmsNH[i,j,}\DecValTok{1}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ x1[[}\DecValTok{1}\NormalTok{]] }\CommentTok{\# mu}
\NormalTok{    RsmParmsNH[i,j,}\DecValTok{2}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ x1[[}\DecValTok{2}\NormalTok{]] }\CommentTok{\# lambda}
\NormalTok{    RsmParmsNH[i,j,}\DecValTok{3}\NormalTok{] }\OtherTok{\textless{}{-}}\NormalTok{ x1[[}\DecValTok{3}\NormalTok{]] }\CommentTok{\# nu}
\NormalTok{  \}}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
The defining values of the fitting model are \texttt{muNH} = 3.3121519, \texttt{lambdaNH} = 1.714368 and \texttt{nuNH} = 0.7036564. These obey the constraints \texttt{lambdaNH\ \textgreater{}\ 0} and \texttt{0\ \textless{}\ nuNH\ \textless{}\ 1}.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aucRocNH }\OtherTok{\textless{}{-}} \FunctionTok{PlotRsmOperatingCharacteristics}\NormalTok{(}
\NormalTok{  muNH, lambdaNH, nuNH, }
  \AttributeTok{lesDistr =}\NormalTok{ lesDistr}\SpecialCharTok{$}\NormalTok{Freq, }\AttributeTok{OpChType =} \StringTok{"ROC"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{aucROC}
\NormalTok{aucwAfrocNH }\OtherTok{\textless{}{-}} \FunctionTok{PlotRsmOperatingCharacteristics}\NormalTok{(}
\NormalTok{  muNH, lambdaNH, nuNH, }
  \AttributeTok{lesDistr =}\NormalTok{ lesDistr}\SpecialCharTok{$}\NormalTok{Freq, }\AttributeTok{OpChType =} \StringTok{"wAFROC"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{aucwAFROC}
\end{Highlighting}
\end{Shaded}

\begin{itemize}
\item
  The plotting function \texttt{PlotRsmOperatingCharacteristics()} returns a number of other objects, most importantly the plot, but here we use only the AUC, which is obtained by numerical integration of the predicted operating characteristic.
\item
...
\begin{center}\includegraphics{19-froc-sample-size_files/figure-latex/unnamed-chunk-7-1} \end{center}
...
The code block applies \texttt{StSignificanceTesting()} to \texttt{rocDataNH} and \texttt{frocDataNH}, using the appropriate FOM, and extracts the variance components \texttt{varComp\_roc} and \texttt{varComp\_afroc}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{temp1 }\OtherTok{\textless{}{-}} \FunctionTok{StSignificanceTesting}\NormalTok{(rocDataNH, }\AttributeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{, }\AttributeTok{method =} \StringTok{"DBM"}\NormalTok{, }\AttributeTok{analysisOption =} \StringTok{"RRRC"}\NormalTok{)}
\NormalTok{temp2 }\OtherTok{\textless{}{-}} \FunctionTok{StSignificanceTesting}\NormalTok{(frocDataNH, }\AttributeTok{FOM =} \StringTok{"wAFROC"}\NormalTok{, }\AttributeTok{method =} \StringTok{"DBM"}\NormalTok{, }\AttributeTok{analysisOption =} \StringTok{"RRRC"}\NormalTok{)}
\NormalTok{varComp\_roc }\OtherTok{\textless{}{-}}\NormalTok{ temp1}\SpecialCharTok{$}\NormalTok{ANOVA}\SpecialCharTok{$}\NormalTok{VarCom}
\NormalTok{varComp\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ temp2}\SpecialCharTok{$}\NormalTok{ANOVA}\SpecialCharTok{$}\NormalTok{VarCom}
\end{Highlighting}
\end{Shaded}

The observed wAFROC effect-size (on the pilot dataset) is -0.00685625. This is a very small effect size as it represent the ``almost'' NH condition; the corresponding ROC effect-size is also small: -0.0051. These are not surprising that the study \citep{zanca2009evaluation} did not find a significant difference between these two treatments

The respective variance components are:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{print}\NormalTok{(varComp\_roc)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{print}\NormalTok{(varComp\_afroc)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{power\_afroc }\OtherTok{\textless{}{-}} \FunctionTok{array}\NormalTok{(}\AttributeTok{dim =} \FunctionTok{length}\NormalTok{(effectSizeROC))}
...
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\SpecialCharTok{:}\FunctionTok{length}\NormalTok{(effectSizeROC)) \{}
  ...
  \CommentTok{\# these are pseudovalue based variance components assuming FOM = "Wilcoxon}
\NormalTok{  varYTR\_roc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_roc[}\StringTok{"VarTR"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{  varYTC\_roc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_roc[}\StringTok{"VarTC"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{  varYEps\_roc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_roc[}\StringTok{"VarErr"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
...
\NormalTok{  ret }\OtherTok{\textless{}{-}} \FunctionTok{SsPowerGivenJK}\NormalTok{(}
    ...
    \AttributeTok{LegacyCode =} \ConstantTok{TRUE}\NormalTok{, }
    \FunctionTok{list}\NormalTok{(}\AttributeTok{VarTR =}\NormalTok{ varYTR\_roc, }\AttributeTok{VarTC =}\NormalTok{ varYTC\_roc, }\AttributeTok{VarErr =}\NormalTok{ varYEps\_roc))}
\NormalTok{  power\_roc[i] }\OtherTok{\textless{}{-}}\NormalTok{ ret}\SpecialCharTok{$}\NormalTok{powerRRRC}
  
  \CommentTok{\# these are pseudovalue based variance components assuming FOM = "wAFROC"}
\NormalTok{  varYTR\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarTR"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{  varYTC\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarTC"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{  varYEps\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarErr"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
...
\NormalTok{  ret }\OtherTok{\textless{}{-}} \FunctionTok{SsPowerGivenJK}\NormalTok{(}
    ...
    \AttributeTok{method =} \StringTok{"DBM"}\NormalTok{, }
    \AttributeTok{LegacyCode =} \ConstantTok{TRUE}\NormalTok{, }
    \FunctionTok{list}\NormalTok{(}\AttributeTok{VarTR =}\NormalTok{ varYTR\_afroc, }\AttributeTok{VarTC =}\NormalTok{ varYTC\_afroc, }\AttributeTok{VarErr =}\NormalTok{ varYEps\_afroc))}
\NormalTok{  power\_afroc[i] }\OtherTok{\textless{}{-}}\NormalTok{ ret}\SpecialCharTok{$}\NormalTok{powerRRRC}
  
  \FunctionTok{cat}\NormalTok{(}\StringTok{"ROC{-}ES = "}\NormalTok{, effectSizeROC[i], }
      \StringTok{", wAFROC{-}ES = "}\NormalTok{, effectSizewAFROC[i], }
      \StringTok{", Power{-}ROC = "}\NormalTok{, power\_roc[i], }
      \StringTok{", Power{-}wAFROC = "}\NormalTok{, power\_afroc[i], }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics{19-froc-sample-size_files/figure-latex/unnamed-chunk-12-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lesDistr }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}\FloatTok{0.69}\NormalTok{, }\FloatTok{0.20}\NormalTok{, }\FloatTok{0.11}\NormalTok{) }\CommentTok{\# sic!}
...
\NormalTok{lesDistr }\OtherTok{\textless{}{-}} \FunctionTok{UtilLesDistr}\NormalTok{(lesDistr) }\CommentTok{\# sic!}
...
The fitting model is defined by \texttt{muNH} = 3.31215193, \texttt{lambdaNH} = 1.71436802 and \texttt{nuNH} = 0.70365644 and \texttt{lesDistr\$Freq} = 0.69, 0.2, 0.11. The effect size scale factor is \texttt{scaleFactor} = 1.26172393. All of these values are consistent with the Part I values.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aucRocNH }\OtherTok{\textless{}{-}} \FunctionTok{PlotRsmOperatingCharacteristics}\NormalTok{(}
\NormalTok{  muNH, lambdaNH, nuNH, }
  \AttributeTok{lesDistr =}\NormalTok{ lesDistr}\SpecialCharTok{$}\NormalTok{Freq, }
  \AttributeTok{OpChType =} \StringTok{"ROC"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{aucROC}

\NormalTok{aucwAfrocNH }\OtherTok{\textless{}{-}} \FunctionTok{PlotRsmOperatingCharacteristics}\NormalTok{(}
\NormalTok{  muNH, lambdaNH, nuNH, }
  \AttributeTok{lesDistr =}\NormalTok{ lesDistr}\SpecialCharTok{$}\NormalTok{Freq, }
  \AttributeTok{OpChType =} \StringTok{"wAFROC"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{aucwAFROC}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{varComp\_afroc  }\OtherTok{\textless{}{-}} \FunctionTok{StSignificanceTesting}\NormalTok{(}
\NormalTok{  frocNhData, }\AttributeTok{FOM =} \StringTok{"wAFROC"}\NormalTok{, }\AttributeTok{method =} \StringTok{"DBM"}\NormalTok{, }
...
\NormalTok{varYEps\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarErr"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{ret }\OtherTok{\textless{}{-}} \FunctionTok{SsPowerGivenJK}\NormalTok{(}
  \AttributeTok{dataset =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{, }\AttributeTok{J =}\NormalTok{ J, }\AttributeTok{K =}\NormalTok{ K, }\AttributeTok{analysisOption =} \StringTok{"RRRC"}\NormalTok{, }
  ...
  \FunctionTok{list}\NormalTok{(}\AttributeTok{VarTR =}\NormalTok{ varYTR\_afroc,}
  \AttributeTok{VarTC =}\NormalTok{ varYTC\_afroc,}
  \AttributeTok{VarErr =}\NormalTok{ varYEps\_afroc))}
\NormalTok{power\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ ret}\SpecialCharTok{$}\NormalTok{powerRRRC}

\FunctionTok{cat}\NormalTok{(}\StringTok{"ROC{-}ES = "}\NormalTok{, ROC\_ES, }
    \StringTok{", wAFROC{-}ES = "}\NormalTok{, ROC\_ES }\SpecialCharTok{*}\NormalTok{ scaleFactor, }
    \StringTok{", Power{-}wAFROC = "}\NormalTok{, power\_afroc, }\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{VarTR\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarTR"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{] }
\NormalTok{VarTC\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarTC"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{VarErr\_afroc }\OtherTok{\textless{}{-}}\NormalTok{ varComp\_afroc[}\StringTok{"VarErr"}\NormalTok{,}\StringTok{"Estimates"}\NormalTok{]}
\NormalTok{ret2 }\OtherTok{\textless{}{-}} \FunctionTok{SsSampleSizeKGivenJ}\NormalTok{(}
  \AttributeTok{dataset =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{J =} \DecValTok{6}\NormalTok{, }
  \AttributeTok{effectSize =}\NormalTok{ effectSizewAFROC, }
  \AttributeTok{method =} \StringTok{"DBM"}\NormalTok{, }\AttributeTok{LegacyCode =} \ConstantTok{TRUE}\NormalTok{,}
  \FunctionTok{list}\NormalTok{(}\AttributeTok{VarTR =}\NormalTok{ VarTR\_afroc, }\AttributeTok{VarTC =}\NormalTok{ VarTC\_afroc, }\AttributeTok{VarErr =}\NormalTok{ VarErr\_afroc))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ret3 }\OtherTok{\textless{}{-}} \FunctionTok{SsPowerGivenJK}\NormalTok{(}
  \AttributeTok{dataset =} \ConstantTok{NULL}\NormalTok{, }\AttributeTok{J =} \DecValTok{6}\NormalTok{, }
  \AttributeTok{K =}\NormalTok{ ret2}\SpecialCharTok{$}\NormalTok{KRRRC, }
  \AttributeTok{effectSize =}\NormalTok{ effectSizewAFROC, }
  \AttributeTok{method =} \StringTok{"DBM"}\NormalTok{, }\AttributeTok{LegacyCode =} \ConstantTok{TRUE}\NormalTok{,}
  \FunctionTok{list}\NormalTok{(}\AttributeTok{VarTR =}\NormalTok{ VarTR\_afroc, }\AttributeTok{VarTC =}\NormalTok{ VarTC\_afroc, }\AttributeTok{VarErr =}\NormalTok{ VarErr\_afroc))}
