...
\section{How much finished 90 percent}\label{froc-sample-size-how-much-finished}}
...
\item
  Part 2 encapsulates some of the details in \texttt{SsFrocNhRsmModel()}.
\item
  Part 3 uses \texttt{SsFrocSampleSize()} which encapsulates the procedure into a single function call which is applied to 7 FROC datasets.
\end{itemize}
...
The following code computes \texttt{JStar} and \texttt{KStar}, the numbers of readers and cases in the \textbf{pilot} dataset. These are needed below for correct scaling of the OR variance components.
...
\hypertarget{fed-dataset-all-data-and-nh-nr-treatments-only}{%
\subsection{FED dataset, all data and NH-NR treatments only}\label{fed-dataset-all-data-and-nh-nr-treatments-only}}

This dataset has five treatments; therefore two results are reported: one using all treatments and one using NH not-rejecting treatments only, i.e., \texttt{DfExtractDataset(ds,\ trts\ =\ c(1,2))}.
...
\hypertarget{lucy-dataset-all-data-and-nh-nr-treatments-only}{%
\subsection{LUCY dataset, all data and NH-NR treatments only}\label{lucy-dataset-all-data-and-nh-nr-treatments-only}}

This dataset has five treatments; therefore two results are reported: one using all treatments and one using NH not-rejecting treatments only, i.e., \texttt{DfExtractDataset(ds,\ trts\ =\ c(1,5))}.
...
\hypertarget{dobbins-1-dataset-all-data-and-nh-nr-treatments-only}{%
\subsection{DOBBINS-1 dataset, all data and NH-NR treatments only}\label{dobbins-1-dataset-all-data-and-nh-nr-treatments-only}}
...
{\centering \includegraphics[width=0.5\linewidth,height=0.2\textheight]{images/froc-sample-size/ds11trt1rdr3} 

}

\caption{RSM fitted ROC plot for dataset11, treatment 1 and reader 3. Note the two defining operating points near the origin. }\label{fig:froc-sample-size-dataset11}
\end{figure}

This dataset has four treatments; therefore two results are reported: one using all treatments and one using NH not-rejecting treatments only, i.e., \texttt{DfExtractDataset(ds,\ trts\ =\ c(1,3,4))}.
...
\hypertarget{dobbins-3-dataset-all-data-and-nh-nr-treatments-only}{%
\subsection{DOBBINS-3 dataset, all data and NH-NR treatments only}\label{dobbins-3-dataset-all-data-and-nh-nr-treatments-only}}

This dataset has four treatments; therefore two results are reported: one using all treatments and one using NH not-rejecting treatments only, i.e., \texttt{DfExtractDataset(ds,\ trts\ =\ c(3,4))}.
...
With one exception, namely \texttt{dataset11}, each analyses demonstrates the considerable power advantage of FROC over ROC analysis. This is due to the increased predicted wAFROC-AUC effect size relative to the postulated ROC-AUC effect size.
