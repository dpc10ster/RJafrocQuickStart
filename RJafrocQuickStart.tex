...
\hypertarget{rsm-fit-the-roc-data}{%
\subsubsection{RSM-fit the ROC data}\label{rsm-fit-the-roc-data}}
...
{\centering \includegraphics{19-froc-sample-size_files/figure-latex/unnamed-chunk-10-1} 

}

\caption{Plot of wAFROC effect size vs. ROC effect size. The straight line fit through the origin has slope 1.2620812.}\label{fig:unnamed-chunk-10}
\end{figure}
...
\textbf{For this dataset the wAFROC effect size is 1.26 times the ROC effect size.} Again, the wAFROC effect size is expected to be larger than the ROC effect size because the intrinsic range of the former \(1-0=1\) is twice that of the latter \(1-0.5=0.5\).
...
The following code applies \texttt{UtilORVarComponentsFactorial()} to \texttt{rocDataNH} and \texttt{frocDataNH} (using ``Wilcoxon'' and ``wAFROC'' FOMs respectively) and extracts the OR variance components.

\begin{Shaded}
\begin{Highlighting}[numbers=left,,]
\NormalTok{varComp\_roc }\OtherTok{\textless{}{-}} \FunctionTok{UtilORVarComponentsFactorial}\NormalTok{(}
\NormalTok{  rocDataNH, }\AttributeTok{FOM =} \StringTok{"Wilcoxon"}\NormalTok{)}\SpecialCharTok{$}\NormalTok{VarCom[}\SpecialCharTok{{-}}\DecValTok{2}\NormalTok{]}
\NormalTok{varComp\_wafroc }\OtherTok{\textless{}{-}} \FunctionTok{UtilORVarComponentsFactorial}\NormalTok{(}
...
\NormalTok{  ret }\OtherTok{\textless{}{-}} \FunctionTok{SsPowerGivenJK}\NormalTok{(}
    ...
    \FunctionTok{list}\NormalTok{(}\AttributeTok{JStar =} \DecValTok{4}\NormalTok{, }\AttributeTok{KStar =} \DecValTok{200}\NormalTok{, }
...
{\centering \includegraphics{19-froc-sample-size_files/figure-latex/unnamed-chunk-15-1} 

}

\caption{Plot of wAFROC power vs. ROC power. For ROC effect size = 0.06 the wAFROC effect size is 0.075724872, the ROC power is 0.55738123 while the wAFROC power is 0.79517494.}\label{fig:unnamed-chunk-15}
\end{figure}
...
\NormalTok{lesDistr }\OtherTok{\textless{}{-}} \FunctionTok{UtilLesDistr}\NormalTok{(frocNhData) }\CommentTok{\# sic!}
...
The fitting model is defined by \texttt{muNH} = 3.3121519, \texttt{lambdaNH} = 1.714368 and \texttt{nuNH} = 0.7036564 and \texttt{lesDistr\$Freq} = 0.69, 0.2, 0.11. The effect size scale factor is \texttt{scaleFactor} = 1.2617239. All of these are identical to the Part I values.
...
The function \texttt{SsPowerGivenJK} used below returns the power for specified effect size, numbers of readers and cases, and variance components (supplied as a \texttt{list} via the \texttt{...} argument). The used default options are:
