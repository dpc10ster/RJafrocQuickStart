...
* The number of lesions per diseased case is the vector `x$lesions$perCase`, i.e., `r x$lesions$perCase` in this example.
* The `x$descriptions$dataType` member is `r x$descriptions$type`, which specifies the data collection method. 
* The `x$descriptions$modalityID` member is a vector with two elements `r x$descriptions$modalityID`, naming the two modalities. 
* The `x$readerID` member is a vector with three elements `r x$descriptions$readerID`, naming the three readers. 
* The `x$descriptions$design` member is `r x$descriptions$design`; specifies the study design. 
