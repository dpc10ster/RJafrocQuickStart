...
Next one determines `lesDistr`, the lesion distribution (the RSM fitting algorithm needs to know how lesion-rich the dataset is as the predicted ROC-AUC depends it). One also needs the lesion weights matrix $\matrix{W}$.
...
Note that `lesDistr` and $\matrix{W}$ are determined from the *FROC* NH dataset as this information is lost upon conversion to an ROC dataset. 


For this dataset fraction `r lesDistr$Freq[1]` of diseased cases have one lesion, fraction `r lesDistr$Freq[2]` of diseased cases have two lesions and fraction `r lesDistr$Freq[3]` of diseased cases have three lesions.  
...
The defining values of the fitting model are `muNH` = `r muNH`, `lambdaNH` = `r lambdaNH` and `nuNH` = `r nuNH`. 
...
* The results are: `aucRocNH = `r aucRocNH`` and `aucwAfrocNH = `r aucwAfrocNH``. Note that the wAFROC-FOM is smaller than the ROC-FOM as it includes detection and localization performance. 
...
```{r, echo = FALSE, fig.show='hold', fig.align='center', fig.cap="Plot of wAFROC effect size vs. ROC effect size."}
