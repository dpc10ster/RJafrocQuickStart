...
```{r, echo = FALSE, fig.show='hold', fig.align='center', fig.cap="Plot of wAFROC effect size vs. ROC effect size. The straight line fit through the origin has slope 1.2620812."}
...
The slope of the zero-intercept constrained straight line fit is `scaleFactor` = `r scaleFactor$coefficients[1]` and the squared correlation coefficient is `R2` = `r summary(scaleFactor)$r.squared` (the fit is very good). Therefore, the conversion from ROC to wAFROC effect size is: 
...
The code applies `StSignificanceTesting()` to `rocDataNH` and `frocDataNH` (using the appropriate FOM) and extracts the variance components.

```{r, attr.source = ".numberLines"}
temp1 <- StSignificanceTesting(rocDataNH, 
           FOM = "Wilcoxon", method = "DBM", analysisOption = "RRRC")
temp2 <- StSignificanceTesting(frocDataNH, 
           FOM = "wAFROC", method = "DBM", analysisOption = "RRRC")
varComp_roc <- temp1$ANOVA$VarCom
varComp_afroc <- temp2$ANOVA$VarCom
```


The wAFROC effect-size on the pilot dataset is `r temp2$RRRC$ciDiffTrt$Estimate`. This is a very small effect size as it represent the "almost" NH condition; the corresponding ROC effect-size is also small: `r temp1$RRRC$ciDiffTrt$Estimate`.

The ROC and wAFROC variance components are:


```{r, echo=FALSE}
varComp_both <- cbind(varComp_roc, varComp_afroc)
...
Only terms involving treatment (i.e., `VarTR`, `VarTC` and `VarErr`) are relevant to sample size. The wAFROC values are larger than the corresponding ROC ones. This is expected because the allowed range of the wAFROC FOM is twice that of the ROC FOM. 
...
```{r, echo=FALSE, fig.show='hold', fig.align='center', fig.cap="Plot of wAFROC power vs. ROC power. For ROC effect size = 0.06 the wAFROC effect size is 0.075724872, the ROC power is 0.55738123 while the wAFROC power is 0.79517494."}
...
### Extract the wAFROC variance components
...
The function `SsPowerGivenJK` used below returns the power for specified effect size, numbers of readers and cases, and variance components.
...
The function `SsSampleSizeKGivenJ` used below returns the number of cases needed for 80 percent power for specified effect size and variance components.
