...
For this dataset `Lmax` is 3, and fraction 0.69 of diseased cases have one lesion, fraction 0.2 of diseased cases have two lesions and fraction 0.11 of diseased cases have three lesions. The `PlotRsmOperatingCharacteristics` function used below sets `relWeights = 0` ensuring equally weighted lesions. 
...
The defining values of the fitting model are `muNH` = 3.3121519, `lambdaNH` = 1.714368 and `nuNH` = 0.7036564. These obey the constraints `lambdaNH > 0` and `0 < nuNH < 1`. 
...
<img src="19-froc-sample-size_files/figure-html/unnamed-chunk-7-1.png" width="672" style="display: block; margin: auto;" />
...
Only terms involving treatment are relevant to sample size. The wAFROC TC variance component, , and the error variance component, ; these values are slightly larger than the corresponding ROC ones:  and , as expected because the range of the wAFROC FOM is twice that of the ROC FOM. 
...
<img src="19-froc-sample-size_files/figure-html/unnamed-chunk-12-1.png" width="672" style="display: block; margin: auto;" />
...
The fitting model is defined by `muNH` = 3.31215193,  `lambdaNH` = 1.71436802  and  `nuNH` = 0.70365644 and `lesDistr$Freq` = 0.69, 0.2, 0.11. The effect size scale factor is `scaleFactor` = 1.26172393. All of these values are consistent with the Part I values.
...
The null hypothesis ROC AUC is 0.87915418 and the corresponding NH wAFROC AUC is 0.71992327. 
